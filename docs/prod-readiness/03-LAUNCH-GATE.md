...
| F6 | System degrades (not fails) when a single upstream — Appwrite, 24SO, Graph, Vipps, OpenAI — is slow or down | ❌ | **New gate (S08).** PR-046/PR-047/PR-050 timeout/fallback remediations are code-complete locally and covered by focused tests, and PR-049's homepage Appwrite-blip crash is code-remediated locally. Gate remains red until deploy smoke verifies those paths and the remaining gaps close: admin loops to login on outage (PR-051), PR-048 deploy log retention/alerting or Sentry/OTel, no dependency-aware readiness probe (PR-055), and remaining upstream deadlines outside the checkout path. |
