...
| S13 | Remediation: money-path + auth-lifecycle + review regressions | ✅ done | 2026-07-02 | — | Independent review verified S05–S12 remediations, then code-remediated: PR-035 atomic stock (decrementRowColumn, min 0, loud oversell), PR-036 own-hold credit in the client stock check, PR-037 scoped+paginated reservation cleanup (+`user_product_idx`), PR-038 `/api/cron/reconcile-orders` sweep (+scheduled-dispatch `ORDERS_RECONCILE_URL`), PR-039 Finago posting via atomic `finago_posting_lock` claim from webhook/return/cron (+missing `finago_transaction_id`/`transition_lock` schema columns), PR-059/PR-060 booking-token atomic claim + interviewer slot-overlap guard (+`claim_lock` column), PR-075 residual pagination (m365-sync prune, turnover invalidation), PR-079 bucket allowlists/size caps + bucket guard test, PR-008 lockfile (stale next@16.2.2 resolution removed). Fixed a review-found regression: anon-session validation passed the session secret to `setJWT` (would log users out on cart actions). Wired `turbo run test` (6 workspaces). All schema changes still need `appwrite push` + owner live checks. |
