...
|---|---|---|
| `mappings/` | Yes (after first review) | `departments.csv` — the reviewed job-department mapping, regenerated by `transform` and the one piece of generated output worth committing, once its `resolved_id` column has actually been filled in. An unreviewed copy carries no information and is not worth keeping. |
