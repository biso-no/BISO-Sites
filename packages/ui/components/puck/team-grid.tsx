    ...
    <div className="w-full py-8">
      <div className={cn("grid gap-8", gridCols[columns])}>
        {members.map((member, index) => {
          if (variant === "card") {
            return (
              <Card key={index} className="p-6 flex flex-col items-center text-center hover:shadow-lg transition-all duration-300 group border-primary/10">
                ...
                <p className="text-sm font-medium text-primary/80 mb-3 uppercase tracking-wide">{member.role}</p>
                {member.bio && (
                ...
                <div className="flex gap-2 mt-auto">
                  ...
                  {member.linkedin && (
                    <Button variant="ghost" size="icon" asChild className="h-8 w-8 rounded-full hover:bg-[#0077b5]/10 hover:text-[#0077b5]">
