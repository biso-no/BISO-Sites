...
const IconMap: Record<string, any> = {
  Sparkles, MapPin, Calendar, Users, Briefcase, Trophy, Megaphone, CheckCircle, ArrowRight, Heart
};
...
export function Hero({
  ...
  const effectiveLayout = layout || (type === "center" || type === "left" || type === "split" ? type : "center");
  ...
  if (effectiveLayout === "carousel") {
    ...
    return (
      ...
      >
           ...
           >
             ...
             {(badge || currentSlide.type) && (
               <div className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white/10 backdrop-blur-md border border-white/20 mb-8">
                 ...
                 <span className="text-white/90">{badge || (currentSlide.type === 'event' ? "Featured Event" : "Featured News")}</span>
               </div>
             )}
                ...
                <div className="flex flex-wrap gap-4 justify-center">
                  {currentSlide.buttons?.map((btn, i) => (
                    <Button 
                      ...
                      asChild 
                      size="lg" 
                      variant={btn.variant === 'glass' ? 'outline' : (btn.variant as any) || 'default'}
                      className={cn(
                        btn.variant === 'gradient' && "bg-linear-to-r from-[#3DA9E0] to-[#001731] text-white border-0",
                        btn.variant === 'glass' && "bg-white/10 backdrop-blur-md border-white/30 text-white hover:bg-white/20"
                      )}
                    >
        ...
        {slides.length > 1 && (
          <>
            <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 text-white cursor-pointer">
              <ChevronLeft className="w-6 h-6" />
            </button>
            <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 text-white cursor-pointer">
            ...
            <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex gap-2 z-20">
              {slides.map((_, idx) => (
                <button
                  ...
                  className={cn("h-2 rounded-full transition-all cursor-pointer", idx === currentIndex ? "bg-white w-8" : "bg-white/40 w-2")}
                />
              ))}
            </div>
          </>
  ...
  return (
    <div className={cn("relative flex flex-col justify-center overflow-hidden w-full", heightClasses[height])}>
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className={cn(
          ...
          effectiveLayout === "split" && "lg:grid lg:grid-cols-2 lg:gap-16 lg:items-center lg:max-w-7xl"
        )}>
          ...
          <div className={cn("flex flex-col gap-6", effectiveLayout === "split" && "lg:text-left")}>
            ...
            {/* Highlights List */}
            {highlights.length > 0 && (
              <div className={cn("grid gap-3 my-2", highlights.length > 2 ? "sm:grid-cols-2" : "grid-cols-1")}>
                {highlights.map((item, idx) => {
                  ...
                  return (
                    <motion.div 
                      ...
                      className={cn("flex items-center gap-2", backgroundImage ? "text-white/90" : "text-foreground/80")}
                    >
            ...
            {buttons.length > 0 && (
              <motion.div 
                ...
                className={cn("flex flex-wrap gap-4 mt-2", activeAlign === "center" && "justify-center")}
              >
                {buttons.map((btn, i) => (
                  <Button
                    key={i}
            ...
            {stats.length > 0 && (
              <motion.div 
                ...
                className={cn("flex flex-wrap gap-3 mt-8", activeAlign === "center" && "justify-center")}
              >
                {stats.map((stat, idx) => (
                   statsVariant === "pills" ? (
                     <div key={idx} className="rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-left shadow-glow backdrop-blur-md">
