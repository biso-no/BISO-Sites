...
export function buildRecruitmentVacancyMetadata(
  ...
  return recruitmentVacancyMetadataSchema.parse({
    ...current,
    company: input.company ?? current.company ?? null,
    employment_type: input.employment_type ?? current.employment_type ?? null,
    paid: input.paid ?? current.paid ?? false,
    short_description:
      input.short_description ?? current.short_description ?? null,
    location: input.location ?? current.location ?? null,
    application_deadline:
      input.application_deadline ?? current.application_deadline ?? null,
    contact_name: input.contact_name ?? current.contact_name ?? null,
    contact_email: input.contact_email ?? current.contact_email ?? null,
    cv_required: input.cv_required ?? current.cv_required ?? false,
  });
}
