    ...
    <article className="mx-auto max-w-4xl px-4 py-12">
      <div className="mb-8 rounded-2xl bg-gradient-to-br from-[#001731] to-[#003366] px-8 py-10 text-white">
        {(props.type || (props.paid !== undefined)) && (
          <div className="mb-3 flex flex-wrap gap-2">
            ...
            {props.paid !== undefined && (
              <span className={`rounded-full px-3 py-0.5 text-xs font-medium ${props.paid ? "bg-emerald-500/20 text-emerald-200" : "bg-gray-500/20 text-gray-200"}`}>
                {props.paid ? "Paid" : "Volunteer"}
              </span>
            )}
          </div>
        )}
        <h1 className="text-3xl font-bold md:text-4xl">{props.title || "Job Title"}</h1>
        {props.department && <p className="mt-2 text-lg text-white/70">{props.department}</p>}
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-8">
          {props.description && (
            <section>
              <h2 className="mb-3 text-xl font-semibold text-gray-900">About the role</h2>
              <p className="text-gray-600 leading-relaxed whitespace-pre-wrap">{props.description}</p>
            </section>
          )}
          {props.responsibilities && props.responsibilities.length > 0 && (
            <section>
              <h2 className="mb-3 text-xl font-semibold text-gray-900">Responsibilities</h2>
    ...
    <article className="mx-auto max-w-5xl px-4 py-12">
      <div className="grid gap-10 lg:grid-cols-2">
        <div className="space-y-3">
          {mainImage ? (
            <div className="overflow-hidden rounded-2xl border border-gray-100 bg-gray-50">
              <img src={mainImage.url} alt={mainImage.alt ?? props.title ?? "Product"} className="h-96 w-full object-cover" />
            </div>
          ) : (
          ...
          {images.length > 1 && (
            <div className="flex gap-2 overflow-x-auto pb-1">
              {images.map((img, i) => (
                <img key={i} src={img.url} alt={img.alt ?? `Image ${i + 1}`} className="h-16 w-16 shrink-0 rounded-lg border border-gray-200 object-cover" />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col">
          <div className="mb-3 flex flex-wrap gap-2">
            {props.badge && (
              <span className="rounded-full bg-blue-100 px-3 py-0.5 text-xs font-semibold text-blue-700">{props.badge}</span>
            )}
            {isOnSale && (
              <span className="rounded-full bg-red-100 px-3 py-0.5 text-xs font-semibold text-red-700">Sale</span>
            )}
            {isOutOfStock && (
              <span className="rounded-full bg-gray-100 px-3 py-0.5 text-xs font-semibold text-gray-600">Out of stock</span>
            )}
          </div>

          <h1 className="text-3xl font-bold text-gray-900">{props.title || "Product Name"}</h1>

          <div className="mt-3 flex items-baseline gap-3">
            {props.price && <span className="text-2xl font-bold text-gray-900">{props.price}</span>}
            {props.originalPrice && <span className="text-base text-gray-400 line-through">{props.originalPrice}</span>}
          </div>
          ...
          {props.description && (
            <p className="mt-4 text-gray-600 leading-relaxed">{props.description}</p>
          )}
          ...
          {props.options && props.options.length > 0 && (
            <div className="mt-6 space-y-4">
              {props.options.map((opt, oi) => (
                <div key={oi}>
                  <label className="mb-1.5 block text-sm font-medium text-gray-700">{opt.name}</label>
                  <div className="flex flex-wrap gap-2">
                    {(opt.values ?? []).map((v, vi) => (
                      <button key={vi} type="button" className="rounded-lg border border-gray-200 px-4 py-2 text-sm transition hover:border-blue-400 hover:text-blue-600">
                        {v.value}
                      </button>
                    ))}
                  </div>
