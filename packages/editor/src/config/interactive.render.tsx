...
export function ContactFormRender(props: ContactFormProps & { variant?: string }) {
      ...
      <section className="mx-auto max-w-5xl py-12 px-4">
        {(props.title || props.subtitle) && (
          <div className="mb-10 text-center">
            {props.title && (
              <h2 className="mb-2 text-3xl font-bold tracking-tight text-gray-900">{props.title}</h2>
            )}
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((card, i) => (
            <div key={i} className="flex flex-col gap-3 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
  ...
  return (
    <section className="mx-auto max-w-5xl py-12 px-4">
      ...
      <div className="grid gap-10 lg:grid-cols-5">
        <form action={props.formAction || "#"} method="POST" className="space-y-6 lg:col-span-3">
        ...
        {(props.contactCards ?? []).length > 0 && (
          <div className="space-y-4 lg:col-span-2">
            {(props.contactCards ?? []).map((card, i) => (
              <div key={i} className="rounded-lg border bg-gray-50 p-5 transition-shadow hover:shadow-sm">
                <div className="mb-2 text-2xl">{iconMap[card.icon] || card.icon}</div>
...
export function MapEmbedRender(props: MapEmbedProps) {
  ...
  return (
    <section className="mx-auto max-w-5xl py-8">
      {props.title && (
        <h2 className="mb-4 text-2xl font-bold tracking-tight">{props.title}</h2>
      )}
