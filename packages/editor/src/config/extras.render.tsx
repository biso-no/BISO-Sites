import { CheckCheck, Download, FileText, Mail, MapPin, Phone } from "lucide-react";
import type { AlertCardProps, ChecklistCardProps, ContactCardsProps, DownloadListProps, NumberedStepsProps, TagListProps } from "./types";
        ...
        <div className="divide-y divide-gray-100 rounded-xl border border-gray-200">
          {items.map((item, i) => (
            <div key={i} className="flex items-center gap-4 px-4 py-3">
              ...
              {item.email && (
                <a href={`mailto:${item.email}`} className="flex items-center gap-1 text-sm text-blue-600 hover:text-blue-800 transition-colors">
    ...
    return (
      <section className="w-full py-10 px-4">
        ...
        <div className="space-y-4">
          {items.map((item, i) => (
            <div key={i} className="flex items-center gap-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              {item.avatar ? (
              ...
              <div className="flex-1">
                ...
                {(item.role || item.department) && (
                  <p className="text-sm text-gray-500">{[item.role, item.department].filter(Boolean).join(" · ")}</p>
                )}
                <div className="mt-2 flex flex-wrap gap-3">
                  {item.email && (
                    <a href={`mailto:${item.email}`} className="flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-800">
                      <Mail className="h-3.5 w-3.5" />
                      {item.email}
                    </a>
                  )}
                  {item.phone && (
                    <a href={`tel:${item.phone}`} className="flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900">
  ...
  return (
    <section className="w-full py-12 px-4">
      ...
      <div className={`grid gap-6 ${colClass[columns] ?? colClass[3]}`}>
        {items.map((item, i) => (
          <div key={i} className="flex flex-col items-center rounded-2xl border border-gray-100 bg-white p-6 text-center shadow-sm transition hover:shadow-md">
            {item.avatar ? (
              <img src={item.avatar} alt={item.name} className="mb-4 h-20 w-20 rounded-full object-cover ring-2 ring-gray-100" />
            ) : (
            ...
            {item.role && <p className="mt-0.5 text-sm text-gray-500">{item.role}</p>}
            {item.department && <p className="mt-0.5 text-xs font-medium text-blue-600">{item.department}</p>}
            <div className="mt-4 flex flex-col gap-2 w-full">
              {item.email && (
                <a href={`mailto:${item.email}`} className="flex items-center justify-center gap-2 rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
                  <Mail className="h-4 w-4 text-gray-400" />
                  Send email
                </a>
              )}
              {item.phone && (
                <a href={`tel:${item.phone}`} className="flex items-center justify-center gap-2 rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
...
export function DownloadListRender({ title, subtitle, variant = "list", items = [] }: DownloadListProps) {
    ...
    return (
      <section className="w-full py-10 px-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item, i) => (
            <a key={i} href={item.url || "#"} download className="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-5 transition hover:shadow-md hover:border-blue-200 group">
              <div className="flex items-start justify-between">
                <FileText className="h-8 w-8 text-gray-400 group-hover:text-blue-500 transition" />
                {item.type && (
                  <span className={`rounded border px-2 py-0.5 text-xs font-semibold uppercase ${typeColor(item.type)}`}>
                    {item.type}
                  </span>
                )}
              </div>
              <div>
                ...
                {item.size && <p className="mt-1 text-xs text-gray-400">{item.size}</p>}
              </div>
...
export function NumberedStepsRender({ title, subtitle, variant = "vertical", steps = [] }: NumberedStepsProps) {
  if (variant === "horizontal") {
    return (
      <section className="w-full py-12 px-4">
        {(title || subtitle) && (
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 max-w-5xl mx-auto">
          {steps.map((step, i) => (
            <div key={i} className="relative rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              ...
              {step.description && <p className="mt-2 text-sm text-gray-500">{step.description}</p>}
            </div>
          ))}
        </div>
      </section>
...
export function TagListRender({ title, variant = "default", size = "md", align = "left", tags = [] }: TagListProps) {
  ...
  return (
    <div className="py-3 px-4">
      {title && <span className="mr-3 text-sm font-medium text-gray-600">{title}</span>}
      <div className={`flex flex-wrap gap-2 ${alignClass[align] ?? "justify-start"} ${title ? "mt-2" : ""}`}>
        {tags.map((tag, i) => {
          ...
          return tag.href ? (
            <a key={i} href={tag.href} className={cls}>{tag.label}</a>
          ) : (
            <span key={i} className={cls}>{tag.label}</span>
          );
        })}
      </div>
    </div>
...
export function AlertCardRender({ title, content, variant = "info", icon = true, ctaLabel, ctaHref }: AlertCardProps) {
  ...
  return (
    <div className={`w-full rounded-xl border p-5 ${s.wrapper}`}>
      <div className="flex gap-3">
        {icon && (
          <svg className={`mt-0.5 h-5 w-5 shrink-0 ${s.icon}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <div className="flex-1">
          ...
          {ctaLabel && ctaHref && (
            <a href={ctaHref} className={`mt-3 inline-block rounded-md px-3 py-1.5 text-sm font-medium transition ${s.btn}`}>
              {ctaLabel}
            </a>
          )}
        </div>
...
export function ChecklistCardRender({
  ...
  return (
    <div className={`w-full ${wrapperClass}`}>
      ...
      <ul className="mt-4 space-y-2.5">
        {items.map((item, i) => (
          <li key={i} className="flex items-start gap-3">
            <span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${item.checked !== false ? "bg-blue-100" : "bg-gray-100"}`}>
              {item.checked !== false ? <CheckIcon /> : <span className="h-2 w-2 rounded-full bg-gray-300" />}
            </span>
            <span className={`text-sm ${item.checked !== false ? "text-gray-700" : "text-gray-500"}`}>
              {item.text}
            </span>
          </li>
        ))}
      ...
      {ctaLabel && ctaHref && (
        <a href={ctaHref} className="mt-5 inline-flex items-center rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-700">
          {ctaLabel}
        </a>
      )}
    </div>
