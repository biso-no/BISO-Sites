...
export function TagListRender({
  ...
  return (
    <div className="px-4 py-3">
      {title && (
        <span className="mr-3 font-medium text-gray-600 text-sm">{title}</span>
      )}
      <div
      ...
      <ul className="mt-4 space-y-2.5">
        {items.map((item, i) => (
          <li className="flex items-start gap-3" key={i}>
            <span
              className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${item.checked === false ? "bg-gray-100" : "bg-blue-100"}`}
            >
              {item.checked === false ? (
