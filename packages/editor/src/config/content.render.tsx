...
import { RichText, type RichTextProps } from "@repo/ui/components/puck/rich-text";
import { TableOfContents, type TableOfContentsProps } from "@repo/ui/components/puck/table-of-contents";
import { Timeline, type TimelineProps } from "@repo/ui/components/puck/timeline";
...
export function TestimonialsRender({ items, variant, columns, title }: TestimonialsProps) {
  ...
  const renderCard = (
    ...
    <div key={index} className="relative rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
      <svg className="absolute top-6 left-6 h-8 w-8 text-gray-200" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex gap-6 overflow-x-auto pb-4 snap-x snap-mandatory">
            {testimonials.map((item, i) => (
              <div key={i} className="min-w-[320px] max-w-[400px] flex-shrink-0 snap-center">
                {renderCard(item, i)}
              </div>
            ))}
          </div>
          {testimonials.length > 1 && (
            <p className="text-center text-xs text-gray-400 mt-4">Scroll to see more</p>
          )}
        </div>
      ) : (
        <div
