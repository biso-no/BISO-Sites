...
export async function getPuckSchema(config: Config<any, any, any>) {
  ...
  for (const [componentType, componentConfig] of componentEntries) {
    ...
    if (componentConfig.fields) {
      ...
      console.log(`Component ${componentType} has ${fieldEntries.length} fields.`);
      ...
      for (const [fieldName, fieldConfig] of fieldEntries) {
        ...
        if (fieldType === "array") {
          ...
          console.log(`    -> Array field detected. arrayFields defined: ${!!(fieldConfig as any).arrayFields}`);
        }

        // Handle object fields
        if (fieldType === "object") {
          field.objectFields = (fieldConfig as any).objectFields || {};
          console.log(`    -> Object field detected. objectFields defined: ${!!(fieldConfig as any).objectFields}`);
        }

        // Handle select/radio options
        if ((fieldConfig as any).options) {
          field.options = (fieldConfig as any).options;
          console.log(`    -> Options detected: ${field.options?.length} options.`);
        }
  ...
  console.log(`Finished getPuckSchema. Total schemas generated: ${schemas.length}`);
  return schemas;
}
 ...
 */
export async function formatSchemaForAI(schemas: ComponentSchema[]): Promise<string> {
