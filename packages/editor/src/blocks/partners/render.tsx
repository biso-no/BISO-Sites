...
interface Props { block: PartnersBlock; edit: boolean; onPatch: PatchFn; }

export function PartnersRender({ block }: Props) {
  const [liveItems, setLiveItems] = useState<PartnerItem[] | null>(null);

  useEffect(() => {
    if (block.source !== "auto") { setLiveItems(null); return; }
    let cancelled = false;
    fetch("/api/pages/partners")
      .then((r) => r.json())
      .then((data: PartnerItem[]) => { if (!cancelled) setLiveItems(data); })
      .catch(() => { if (!cancelled) setLiveItems(null); });
    return () => { cancelled = true; };
  }, [block.source]);

  const items = (block.source === "auto" ? liveItems : null) ?? block.items ?? [];
  ...
  return (
    <div className="pg-partners pg-block">
      ...
      <div className="pg-partners__grid">
        {items.map((p, i) => (
          <div key={i} className="pg-partners__logo">
            {p.logoSrc ? (
        ...
        {items.length === 0 && (
          <p style={{ fontSize: 13, color: "var(--ink-3)", gridColumn: "1/-1" }}>
            {block.source === "auto" ? "Loading partners…" : "No partners added yet."}
          </p>
        )}
      </div>
    </div>
