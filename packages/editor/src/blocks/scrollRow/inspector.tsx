...
import type { ScrollRowBlock, ScrollRowItem, PageDoc } from "@/editor/types";
import type { PatchFn } from "@/blocks/types";
import { InspRow, InspSection } from "@/components/editor-shell/inspector/insp-parts";

interface Props { block: ScrollRowBlock; doc: PageDoc; onPatch: PatchFn; }
    ...
    <>
      <InspSection label="Content">
        <InspRow label="Heading">
          <input value={block.heading ?? ""} onChange={(e) => onPatch("heading", e.target.value)} placeholder="Benefits" />
        </InspRow>
      </InspSection>
      <InspSection label={`Cards (${block.items.length})`}>
        {block.items.map((item, i) => (
          <div key={i} style={{ padding: "8px 0", borderBottom: "0.5px solid var(--rule)" }}>
            ...
            <InspRow label="Icon"><input value={item.icon ?? ""} onChange={(e) => patchItem(i, { icon: e.target.value || undefined })} placeholder="★" /></InspRow>
        ...
        <button
          ...
          onClick={() => onPatch("items", [...block.items, { title: "New benefit", body: "Description." }])}
        >+ Add card</button>
      </InspSection>
    </>
  );
