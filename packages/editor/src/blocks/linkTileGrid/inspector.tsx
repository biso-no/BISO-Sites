...
interface Props { block: LinkTileGridBlock; doc: PageDoc; onPatch: PatchFn; }
      ...
      <InspSection label={`Tiles (${block.items.length})`}>
        {block.items.map((item, i) => (
          <div key={i} style={{ padding: "8px 0", borderBottom: "0.5px solid var(--rule)" }}>
            ...
            <InspRow label="Description"><input value={item.description ?? ""} onChange={(e) => patchItem(i, { description: e.target.value || undefined })} /></InspRow>
            <InspRow label="URL"><input value={item.href} onChange={(e) => patchItem(i, { href: e.target.value })} placeholder="https://…" /></InspRow>
          </div>
        ))}
        <button
          type="button"
          ...
          onClick={() => onPatch("items", [...block.items, { icon: "→", title: "New page", href: "#" }])}
        >+ Add tile</button>
      </InspSection>
    </>
  );
