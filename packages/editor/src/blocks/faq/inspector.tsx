    ...
    <>
      ...
      <InspSection label="FAQ">
        ...
        {block.items.map((item, i) => (
          <div key={i} style={{ padding: "6px 0", borderBottom: "0.5px solid var(--rule)", display: "flex", alignItems: "flex-start", gap: 6 }}>
            <div style={{ flex: 1 }}>
              <InspRow label="Q">
                <input value={item.q} onChange={(e) => {
                  onPatch("items", block.items.map((x, j) => j === i ? { ...x, q: e.target.value } : x));
                }} />
              </InspRow>
              <InspRow label="A">
                <textarea value={item.a} rows={2} onChange={(e) => {
                  onPatch("items", block.items.map((x, j) => j === i ? { ...x, a: e.target.value } : x));
                }} />
              </InspRow>
            </div>
            <button
              ...
              style={{ flexShrink: 0, width: 18, height: 18, borderRadius: "50%", border: 0, background: "var(--rule-2)", cursor: "pointer", fontSize: 9, display: "grid", placeItems: "center", marginTop: 4 }}
