...
interface Props { block: EventsBlock; edit: boolean; onPatch: PatchFn; }
...
export function EventsRender({ block, edit, onPatch }: Props) {
  ...
  useEffect(() => {
    if (!dept) { setLiveItems(null); return; }
    let cancelled = false;
    setLoading(true);
    fetch(`/api/pages/events?dept=${encodeURIComponent(dept)}`)
      .then((r) => r.json())
      .then((data: EventItem[]) => { if (!cancelled) setLiveItems(data); })
  ...
  return (
    <div className="pg-events pg-block">
      ...
      <div className="pg-events-grid">
        {items.map((ev, i) => (
          <div key={i} className="pg-event-card">
        ...
        {items.length === 0 && (
          <p style={{ fontSize: 13, color: "var(--ink-3)", gridColumn: "1/-1" }}>
            {loading ? "Loading…" : isLive ? "No upcoming events." : "Set a department to load live events."}
          </p>
        )}
      </div>
    </div>
