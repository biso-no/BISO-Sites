...
interface Props { block: FeatureGridBlock; edit: boolean; onPatch: PatchFn; }
...
export function FeatureGridRender({ block, edit }: Props) {
  ...
  return (
    <div className={`pg-featuregrid pg-featuregrid--${variant} pg-block`}>
      ...
      <div className="pg-featuregrid__grid" style={{ gridTemplateColumns: `repeat(${cols},1fr)` }}>
        {block.items.map((item, i) => (
          <div key={i} className="pg-featuregrid__card">
            <div className="pg-featuregrid__icon" aria-hidden="true">{item.icon}</div>
            <div className="pg-featuregrid__title">{item.title}</div>
            <div className="pg-featuregrid__body">{item.body}</div>
            {item.href && !edit ? <a className="pg-featuregrid__link" href={item.href}>Learn more →</a> : null}
          </div>
        ))}
      </div>
    </div>
