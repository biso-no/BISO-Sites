...
export async function syncMembershipsFrom24SO(): Promise<MembershipProductSyncResult> {
    ...
    try {
        ...
        console.log(`[Membership Sync] Found ${products.length} membership products`);
        ...
        const membershipCategories = categories.filter(
            (c) => c.Name?.includes("BISO Membership")
        );
        console.log(`[Membership Sync] Found ${membershipCategories.length} membership categories`);
        ...
        console.log(`[Membership Sync] Completed: ${result.created} created, ${result.updated} updated, ${result.skipped} skipped`);
 ...
 */
export async function previewMembershipSync(): Promise<MembershipProductSyncItem[]> {
