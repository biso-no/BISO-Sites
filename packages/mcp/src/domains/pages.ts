  ...
  z.object({
    ...
    path: z
      ...
      .refine((value) => propPathProblem(value) === null, {
        message:
          "A prop path may not start with `id` or `type` (the block's own identity), contain `__proto__`, `constructor`, `prototype` or `length`, exceed 12 segments or 64 characters per segment, or address an array index above 999.",
      })
