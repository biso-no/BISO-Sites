    ...
    try {
        const order = await databases.createRow(
            ...
            orderId,
            {
                ...
                buyer_name: params.customerInfo?.firstName && params.customerInfo?.lastName 
...
async function updateOrderStatus(
    ...
    switch (paymentState.state) {
        ...
        case "AUTHORIZED":
            ...
            updateData.vipps_order_id = sessionData.payment?.aggregate?.authorizedAmount?.value?.toString() || null
            break
        case "ABORTED":
...
export async function createCheckoutSession(
    ...
    try {
        ...
        const checkoutRequest = {
            ...
            ...(params.customerInfo && Object.keys(params.customerInfo).length > 0 && {
                prefillCustomer: {
                    ...
                    ...(params.customerInfo.streetAddress && { streetAddress: params.customerInfo.streetAddress }),
...
export async function verifyOrderStatus(
    ...
    try {
        ...
        const accessToken = await getAccessToken()
        const sessionInfo = await client.checkout.info(merchantSerialNumber, accessToken, order.vipps_session_id)
