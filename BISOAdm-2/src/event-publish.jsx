// Event publish flow — same dialog chrome as jobs, copy & stats event-flavoured.
...
/* Event seal — uses a date plaque instead of the bare initial */
.publish-seal{
  ...
  box-shadow: 0 12px 32px rgba(26,24,20,.25);
  overflow: hidden;
}
.publish-seal .m{
...
.publish-seal .d{
  ...
  font-size: 56px; line-height: 1; padding-top: 14px;
...
function EventPublishProgress({ done }) {
  ...
  const stages = [
    "Validating dates and venue",
    "Saving Norwegian translation",
    "Provisioning ticket inventory",
    "Indexing for 4,217 subscribers",
    "Pushing to biso.no and the app",
  ];
  return (
...
function EventPublishCelebration({ draft, onClose }) {
  const month = draft.startDate ? fmtMonth(draft.startDate) : "TBD";
  const day = draft.startDate ? fmtDayNum(draft.startDate) : "—";
  const dayName = draft.startDate ? new Date(draft.startDate).toLocaleDateString("en-GB", { weekday: "short" }) : "";
  const daysOut = daysUntil(draft.startDate);
  return (
    <div className="publish-overlay">
      <div className="publish-card">
        <EvConfetti />
        <div className="publish-seal">
          <div className="m">{month}</div>
          <div className="d">{day}</div>
          <div className="day">{dayName}</div>
        </div>
        <h1>On the <em>calendar</em>.</h1>
        <p className="lede">
          "{draft.titleEn}" is live at biso.no and is now opening for sign-ups in the BISO app.
        </p>

        <div className="publish-stats">
          <div><b>4,217</b><span>Notified</span></div>
          <div><b>{daysOut != null ? `${daysOut}d` : "—"}</b><span>Until doors open</span></div>
          <div><b>{draft.capacity || "—"}</b><span>Seats on sale</span></div>
        </div>

        <div className="publish-links">
          <span className="url"><span>https://biso.no/oslo/events/</span>{draft.slug}</span>
          <button><I.link size={11}/> Copy link</button>
          <button><I.eye size={11}/> Open</button>
        </div>

        <div className="publish-actions">
          <button className="btn" onClick={onClose}>Back to events</button>
        ...
        <div className="next-up">
          ...
          <div className="items">
            <div>
              <b>Add to Facebook</b>
              <span>Auto-creates a public event</span>
            </div>
            <div>
              <b>Feature on Friday newsletter</b>
              <span>Editor cutoff: Thu 18:00</span>
            </div>
            <div>
              <b>Brief the volunteers</b>
              <span>8 ESN buddies opted in for shifts</span>
            </div>
          </div>
        </div>
      </div>
...
(function(){ const s = document.createElement("style"); s.textContent = evPublishCSS; document.head.appendChild(s); })();
window.EventPublishProgress    = EventPublishProgress;
window.EventPublishCelebration = EventPublishCelebration;
