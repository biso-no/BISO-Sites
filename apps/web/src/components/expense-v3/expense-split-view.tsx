...
import { createExpense, createExpenseAttachment, uploadExpenseAttachment } from "@/lib/actions/expense";
...
export function ExpenseSplitView({
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      store.updateReceipt(tempId, {
        status: "ready",
        progress: 100,
        description: data.description || data.vendor || `Receipt from ${file.name}`,
        amount: (data.currency !== "NOK" && data.amountInNok) ? data.amountInNok : (data.amount ?? 0),
  ...
  return (
    <div className="flex h-dvh w-full flex-col overflow-hidden bg-gray-50 dark:bg-[#0B1120] md:flex-row">
      ...
      <div
        className={cn(
          "w-full shrink-0 border-r border-white/10 bg-[#0B1120] md:w-[350px] lg:w-[400px]",
      ...
      >
        {store.selectedReceiptId ? (
          <div className="relative h-full pt-16">
            ...
            {store.receipts.find(r => r.id === store.selectedReceiptId) && (
              <GenerativeReceiptPreview
                receipt={store.receipts.find(r => r.id === store.selectedReceiptId)!}
                onUpdate={(updates) => store.updateReceipt(store.selectedReceiptId!, updates)}
              />
            )}
          </div>
        ) : (
          <ExpenseReport
            ...
            onInsert={(afterId, receipt) => store.insertReceiptAfter(afterId, receipt)}
            ...
            onAssign={(c, d) => store.setAssignment({
              campusId: c,
              departmentId: d,
              campusName: "",
              departmentName: ""
