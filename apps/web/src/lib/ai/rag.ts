...
export const searchSharePoint = tool({
  description:
    "Search the SharePoint knowledge base and return top matching passages with citations. Supports both semantic search and exact matching for specific references like paragraph numbers (§ 6.3, paragraf 6.3, etc.)",
    ...
    try {
      ...
      return {
        error:
          error instanceof Error
            ? error.message
            : "Unknown error occurred while getting statistics",
      };
    }
  },
});
