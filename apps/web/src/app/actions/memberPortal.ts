...
import { PublicProfiles, Users, MemberBenefit } from "@repo/api/types/appwrite"
...
export async function createOrUpdatePublicProfile(profile: Partial<PublicProfiles>, isPublic: boolean): Promise<PublicProfiles | null> {
    ...
    return await db.upsertRow<PublicProfiles>('app', 'public_profiles', profile.$id, {
        ...
        $permissions: [
            isPublic ? Permission.read("any") : Permission.read("user:" + profile.user_id!),
        ]
    });
}
...
export async function updatePublicProfile(data: Partial<PublicProfiles>): Promise<PublicProfiles | null> {
    try {
...
export async function uploadAvatar(formData: FormData): Promise<{ success: boolean; fileId?: string; url?: string; error?: string }> {
    try {
