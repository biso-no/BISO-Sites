...
export async function MemberPortalContent({ user, membership, hasBIIdentity }: MemberPortalContentProps) {
  ...
  const [benefits, revealedBenefits, estimatedSavings] = isMember ? await Promise.all([
  ...
  const expiryDate = membership.membership?.expiryDate || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString()
  const startDate = new Date(new Date(expiryDate).getTime() - 365 * 24 * 60 * 60 * 1000).toISOString()
  const daysRemaining = Math.floor((new Date(expiryDate).getTime() - Date.now()) / (24 * 60 * 60 * 1000))
