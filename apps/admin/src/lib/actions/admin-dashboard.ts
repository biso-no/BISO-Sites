...
type SystemAlertMetric = { id: number; type: 'error' | 'warning' | 'info' | 'success'; message: string; timestamp: string }
...
export async function getDashboardMetrics(): Promise<DashboardMetrics> {
  ...
  const [
    ...
    safeFetch('notices', () =>
      fetchRows<AppNotices>(db, NOTICES_TABLE, [
        Query.select(['$id', 'title', 'description', 'color', 'priority', 'isActive', '$updatedAt']),
        Query.equal('isActive', true),
        Query.orderDesc('priority'),
      ], { maxRows: 20 }),
    ),
    safeFetch('audit logs', () =>
      fetchRows<AuditLogs>(db, AUDIT_LOGS_TABLE, [
  ...
  const uniqueJobIds = Array.from(new Set(jobApplications.map((app) => app.job_id).filter(Boolean)))

  const [jobs, jobTranslations] = await Promise.all([
    uniqueJobIds.length
      ? safeFetch('jobs', () =>
          fetchRows<Jobs>(db, JOBS_TABLE, [
    ...
    uniqueJobIds.length
      ? safeFetch('job translations', () =>
          fetchRows<ContentTranslations>(db, CONTENT_TRANSLATIONS_TABLE, [
...
function buildJobApplicationMetrics(
  ...
  return Array.from(counts.entries())
    .map(([jobId, total]) => {
      ...
      const openPositions = metadata?.openPositions && metadata.openPositions > 0
...
function parseOrderItems(payload?: string | null): ParsedOrderItem[] {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed.filter((item) => typeof item === 'object' && item !== null) as ParsedOrderItem[]
    }
  } catch (error) {
...
function resolveAlertType(color?: string | null): SystemAlertMetric['type'] {
  ...
  if (normalized.includes('yellow') || normalized.includes('orange') || normalized.includes('amber')) {
    return 'warning'
  }
