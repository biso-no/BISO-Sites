...
export async function getDepartmentsClient(params: GetDepartmentsParams & { limit?: number; offset?: number } = {}) {
  ...
  const transformedDepartments = departments.map(dept => {
    ...
    const enTranslation = dept.translations?.find(t => t.locale === 'en') || dept.translations?.[0];
...
export async function createDepartment(data: {
  ...
  const translationRefs: ContentTranslations[] = data.translations.map(t => ({
