...
import type { Notification, NotificationType } from '@/components/notifications/notifications-dropdown'
...
export async function fetchNotifications(): Promise<Notification[]> {
  try {
    ...
    const response = await db.listRows<AppNotices>(
      DATABASE_ID,
      NOTICES_TABLE,
      [
...
function mapColorToType(color?: string | null): NotificationType {
  ...
  if (normalized.includes('yellow') || normalized.includes('orange') || normalized.includes('amber') || normalized.includes('warning')) {
    return 'warning'
  }
