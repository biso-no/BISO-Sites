...
export async function syncM365Permissions(userId: string) {
    ...
    try {
        ...
        if (!microsoftIdentity || !microsoftIdentity.providerAccessToken) {
        ...
        const graphResponse = await fetch(
            ...
            {
                headers: { Authorization: `Bearer ${microsoftIdentity.providerAccessToken}` },
            }
        );

        const graphData = await graphResponse.json();
        if (!graphResponse.ok) throw new Error(`Graph Error: ${JSON.stringify(graphData)}`);
        ...
        for (const group of azureGroups) {
            ...
            if (!name.startsWith("SG-App-")) continue;
            ...
            if (name.startsWith("SG-App-Role-")) {
                ...
                if (roleName === "globaladmin") isGlobalAdmin = true;
            }

            // CASE B: Scopes (SG-App-Campus-* or SG-App-Dept-*)
            else if (name.startsWith("SG-App-Campus-") || name.startsWith("SG-App-Dept-")) {
                teamsToSync.push({
                    id: group.id, // We use the Azure GUID as the Appwrite Team ID
                    name: name,   // We use the Azure Name as the Team Name
                });
            }
        }
        ...
        console.log(`Synced User ${userId}: ${teamsToSync.length} Teams, Roles: [${roles}]`);
