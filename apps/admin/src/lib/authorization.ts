 ...
 */
const CAMPUS_NAMES = ["National", "Oslo", "Bergen", "Stavanger", "Trondheim"] as const;
 ...
 */
function isNationalOperations(campusNames: string[], departmentNames: string[]): boolean {
    return campusNames.includes("National") && departmentNames.includes("OperationsUnit");
}
 ...
 */
function getManagedCampuses(campusNames: string[], departmentNames: string[]): string[] {
...
export async function getUserAuthContext(): Promise<UserAuthContext | null> {
    try {
        ...
        if ((labels.includes("admin") || labels.includes("globaladmin")) && !roles.includes("globaladmin")) {
            roles.push("globaladmin");
        }

        // Derive globaladmin from National + OperationsUnit combination
        if (isNationalOperations(campusNames, departmentNames) && !roles.includes("globaladmin")) {
            roles.push("globaladmin");
        }
