...
import { previewSync, executeSync } from "@/app/actions/membership-sync";
...
export function MembershipSyncClient({ initialItems }: MembershipSyncClientProps) {
    const [items, setItems] = useState<MembershipProductSyncItem[]>(initialItems);
    const [syncResult, setSyncResult] = useState<MembershipProductSyncResult | null>(null);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-4">
                ...
                <Card className="glass-panel">
                    <CardHeader className="pb-2">
                        <CardTitle className="font-medium text-sm text-emerald-600 dark:text-emerald-400">Active</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="glass-panel">
                    <CardHeader className="pb-2">
                        <CardTitle className="font-medium text-sm text-rose-600 dark:text-rose-400">Expired</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="font-bold text-2xl text-rose-600 dark:text-rose-400">{expiredCount}</div>
                    </CardContent>
                </Card>
                <Card className="glass-panel">
                    <CardHeader className="pb-2">
                        <CardTitle className="font-medium text-sm text-amber-600 dark:text-amber-400">No Category</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="font-bold text-2xl text-amber-600 dark:text-amber-400">{unmatchedCount}</div>
                    </CardContent>
                </Card>
            </div>
                ...
                <AlertDialog>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                This will sync {items.length} membership products from 24SevenOffice
                                to the Appwrite database. Existing records will be updated.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
            ...
            <Card className="glass-panel">
                <CardHeader>
                    ...
                    <CardDescription>
                        Products from 24SevenOffice that will be synced to the memberships table.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <Table>
                        ...
                        <TableBody>
                            {items.map((item) => (
                                <TableRow key={item.productId}>
                                    ...
                                    <TableCell>
                                        {item.categoryId ? (
                                            <span className="font-mono text-sm">{item.categoryId}</span>
                                        ) : (
