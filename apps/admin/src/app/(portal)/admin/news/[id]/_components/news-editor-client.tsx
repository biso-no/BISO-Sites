...
import { createNews, updateNews, newsSchema, type NewsFormValues } from "../../../_actions/news";
...
const STATUS_OPTIONS = [{ value: "draft", label: "Draft" }, { value: "published", label: "Published" }];
const LOCALE_OPTIONS = [{ value: "no", label: "Norwegian" }, { value: "en", label: "English" }];
...
export function NewsEditorClient({ article, campuses, isNew, labels }: NewsEditorClientProps) {
  ...
  if (translation?.additional_fields) {
    try { category = JSON.parse(translation.additional_fields).category ?? null; } catch { /* ignore */ }
  }
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const result = isNew ? await createNews(validated.data) : await updateNews(article!.$id, validated.data);
  ...
  return (
    <div className="pb-12">
      <EditorHeader backHref="/admin/news" backLabel={labels.back} title={isNew ? "New Article" : (translation?.title ?? "Edit Article")} status={isNew ? undefined : article?.status}>
        <PortalButton variant="ghost" size="sm" onClick={() => router.push("/admin/news")}>{labels.discard}</PortalButton>
            ...
            {(field) => (
              <PortalField label={labels.title} required>
                <textarea
                  ...
                  onBlur={() => { field.handleBlur(); if (isNew && !form.getFieldValue("slug")) form.setFieldValue("slug", generateSlug(field.state.value)); }}
                  ...
                  style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "#fff", fontFamily: "serif" }}
                  onFocus={(e) => { e.currentTarget.style.borderColor = "rgba(61,169,224,0.50)"; }}
                />
              </PortalField>
            )}
          </form.Field>
            ...
            <form.Field name="author">
              {(field) => (
                <PortalField label={labels.author}>
                  <PortalInput value={field.state.value ?? ""} onChange={(e) => { field.handleChange(e.target.value || null); setPreviewAuthor(e.target.value); }} placeholder="Author name..." />
                </PortalField>
              )}
            </form.Field>
            ...
            <form.Field name="locale">
              {(field) => (
                <PortalField label={labels.locale}>
                  <PortalSelect value={field.state.value} onChange={(e) => field.handleChange(e.target.value as "no" | "en")} options={LOCALE_OPTIONS} />
                </PortalField>
              )}
            </form.Field>
          </div>
          ...
          <form.Field name="description">
            {(field) => (
              <PortalField label={labels.body}>
                <PortalTextarea rows={10} value={field.state.value ?? ""} onChange={(e) => field.handleChange(e.target.value || null)} placeholder="Write your article here..." />
              </PortalField>
            )}
          </form.Field>
            ...
            <form.Field name="status">
              {(field) => (
                <PortalField label={labels.status}>
                  <PortalSelect value={field.state.value} onChange={(e) => field.handleChange(e.target.value as "draft" | "published")} options={STATUS_OPTIONS} />
                </PortalField>
              )}
            </form.Field>
          </div>
        ...
        <div className="lg:sticky lg:top-32 self-start">
          <PreviewPanel title={labels.preview}>
            <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
              {previewImage && <img src={previewImage} alt="" className="w-full h-28 object-cover" />}
              <div className="p-4">
                <p className="font-medium text-sm leading-snug" style={{ color: "#fff", fontFamily: "serif" }}>{previewTitle || "Article Headline"}</p>
                <p className="text-xs mt-2" style={{ color: "rgba(255,255,255,0.40)" }}>{previewAuthor || "Author"} · {new Date().toLocaleDateString()}</p>
              </div>
            </div>
          </PreviewPanel>
        </div>
