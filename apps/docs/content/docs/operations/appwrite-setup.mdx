...
<Steps>
  ...
  <Step title="Import collections & storage buckets">
    Manually create collections and buckets (`events`, `product-images`, `documents`, `recruitment_resumes`) with the permissions outlined below. Double-check indexes for orders, events, translations, and recruitment submissions.
  </Step>
...
1. Create storage buckets:
   ...
   - `recruitment_resumes` - Private PDF CV uploads for job applications

2. Configure permissions:
   - Read: `any` (public access)
   - Write: `users` (authenticated only)

For `recruitment_resumes`, do not grant public read access. Enable file security, allow only `pdf`, and keep the maximum file size at `5 MB`.
