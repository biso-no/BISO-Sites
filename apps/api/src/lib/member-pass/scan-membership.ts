...
/**
 * Live Finago membership for an app scan: this app's own `unstable_cache`
 * reader (own cache key, own revalidate tag) plumbed into the shared
 * freshness rule (`@repo/shared/member-pass/scan-membership`). See that
 * module for why a stale cache entry is treated as a miss instead of being
 * served — a door scanner cannot afford a Finago outage to look like a
 * valid pass.
 *
